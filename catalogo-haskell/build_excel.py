...
notas = [
    ...
    ("Alcance de esta muestra", "20 productos reales, en 6 lineas (Cavalo Forte, Bendito Loiro, Matcha, Murumuru, Frizz Cancelado, Cachos Sim!), de un catalogo que tiene muchas mas lineas y decenas de kits adicionales. Esto es una MUESTRA REAL para validar la estructura, no el catalogo completo — igual que aclara la seccion 19 de la especificacion original."),
    ("Como se obtuvo", "Navegacion directa a las paginas publicas de cada linea y lectura del texto renderizado (nombre, precio en BRL, descripcion de linea, activos, beneficios). No se uso el exportador Playwright del paquete original; para el catalogo completo (incluyendo kits, todas las lineas, colecciones de coloracion y variantes) corresponde ejecutar 'npm run scrape' del exportador ya entregado."),
    ("Precios", "Todos los precios son los publicados en BRL (real brasileno) al momento de la extraccion. Ninguno de los 20 productos mostraba precio de oferta/promocional en la pagina de linea — por eso precio_oferta_brl queda vacio y en_oferta = FALSO para todos. No se inventaron descuentos."),
    ...
    ("Traduccion", "Traduccion manual al espanol neutro latinoamericano, fiel al contenido publicado en portugues (conservando activos, marca, nombres de linea e ingredientes). No se agregaron beneficios ni propiedades que no aparecieran en la fuente. Se evito el voseo argentino (tenes/podes) y el che; se uso el modo impersonal/infinitivo en las instrucciones de uso, valido en toda America Latina."),
    ("Ingredientes INCI", "No se incluyen en esta muestra: la lista de ingredientes tecnicos (INCI) solo aparece en la ficha individual de cada producto, no en la pagina de linea. Columna 'ingredientes_originales' queda vacia; se completa al correr el exportador completo, que si visita cada ficha."),
    ("Imagenes", "No se descargaron ni referenciaron URLs de imagen en esta pasada (las paginas de linea muestran imagenes por CSS/carrusel, no como URL de producto individual estable). Las columnas de imagen quedan vacias; se completan con el exportador completo (seccion 5 de la especificacion, DOWNLOAD_IMAGES=true)."),
    ("Deduplicacion", "Clave usada en esta muestra: slug generado a partir de nombre_es (linea 15 de la especificacion original). El esquema Postgres (esquema_postgres.sql) declara slug UNIQUE y sku UNIQUE (nullable) para que la carga real deduplique igual."),
    ...
    ("Proximo paso sugerido", "1) Revisar y aprobar esta estructura y muestra. 2) Ejecutar el exportador completo (npm run scrape) para las demas lineas y kits. 3) Cargar el resultado en el esquema Postgres adjunto (esquema_postgres.sql). 4) Vincular cada 'sku' resultante con CatalogLine.sku del backend de la plataforma comercial."),
]
r = 1
